...
public class ContextAwareMinIntLabel extends JLabel implements IContextUpdateListener {

    private static final String LABEL = "max: ";

    public ContextAwareMinIntLabel() {
        super();
        setText(LABEL + 0);
        subscribe(this);
    }
    ...
    public void processUpdate(BucketContext context) {
        if (context.getMinIntensity().hasChanged()) {
            setText(LABEL + context.getMinIntensity().getCurrent());
            repaint();
        }
    }
}
